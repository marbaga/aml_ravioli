...
def extract_wavelet_features(x, waveletname='db4'): #extract 34 statistical features on each coefficient set (also percentiles on the approximation coefficients)
    features = []
    list_coeff = pywt.wavedec(x, waveletname, level=4)
    for coeff in list_coeff:
        for index in [np.mean(coeff), np.median(coeff), np.std(coeff), np.var(coeff), calculate_entropy(coeff), np.log(calculate_entropy(coeff))]:
            features.append(index)
    return features + get_percentiles(list_coeff[4])

def extract_pqrs_features(x): #should extract features from pqrst complexes
    #implement this
    return []
...
for i, row in enumerate(X_t):
    ...
    try:
        ...
        features += extract_wavelet_features(x)
        features += extract_pqrs_features(x)
        X.append(features)
        print(features)
...
pd.DataFrame(X).to_csv('X_test_manual_extraction2.csv')
X = pd.read_csv('X_test_manual_extraction2.csv', ',').iloc[:, 1:].to_numpy()
