...
# use lof
method = LocalOutlierFactor(n_neighbors=max(50, int(0.1*X_t.shape[1])), contamination=0.08)
...
####################################################
new = pd.read_csv('task1/results/to_keep8.csv', ',').to_numpy().flatten()
...
#poly.fit(selected_X)
#selected_X = poly.transform(selected_X)  # creating polynomial features
